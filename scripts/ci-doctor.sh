...
grep -q 'bash ops/ci/security.sh' "$repo_root/.github/workflows/ci.yml" || fail "github ci workflow must run the security lane"
grep -q 'bash ops/ci/jankurai.sh' "$repo_root/.github/workflows/ci.yml" || fail "github ci workflow must run the jankurai lane"
...
grep -q 'bash ops/ci/pr-ci.sh' "$repo_root/.github/workflows/release.yml" || fail "github release workflow must run the pr-ci lane"
grep -q 'bash ops/ci/release.sh' "$repo_root/.github/workflows/release.yml" || fail "github release workflow must run the release lane"
...
grep -q -- '--verify-tag' "$repo_root/.github/workflows/release.yml" || fail "github release workflow must bind the release to a verified tag (--verify-tag)"
